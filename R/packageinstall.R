package.install <- function(packages=pizzR::dependencies, verbose = 1){
  ...
  if (any(to_install)){
    missing_packages <- packages[to_install]
    ...
    success <- (all(packages %in% rownames(installed.packages())))
    if (success) cat("\nDone.\n\n")
    if (!success){
      to_install <- !packages %in% rownames(installed.packages())
      missing_packages <- as.character(packages[to_install])
