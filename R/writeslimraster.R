writeslimRaster <- function(x, filename, compression=T, overwrite=T, BIGTIFF="YES", filetype="GTiff", samplesize=100, datatype=NULL, NAflag=NULL, ...){
  ...
  if (rsttype == "SpatRaster"){
    ...
    if (!compression) cat(paste0("\n", Sys.time(), ": Write rasterfile as '", fparameters$datatype,"' ...\n"))
